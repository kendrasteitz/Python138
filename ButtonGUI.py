# Exercise No.   1
# File Name:     SquareButton.py
# Programmer:    Kendra Steitz
# Date:          November 5, 2016
#
# Problem Statement:
#   Create GUI with usable buttons from Button class for
...
def main():
    ...
    maleButton = Button(win, Point(6, 12), 6, 2, "Male")
    femaleButton = Button(win, Point(14, 12), 6, 2, "Female")
    maleButton.activate()
    femaleButton.activate()

    calculate = Button(win, Point(10, 8), 12, 2, "Calculate")
    ...
    while(motherHeight <= 0 or fatherHeight <= 0):
        Text(Point(10, 5), "Please enter a height for each parent.").draw(win)
        pt = win.getMouse()
        motherHeight = eval(inputMother.getText())
        fatherHeight = eval(inputFather.getText())
